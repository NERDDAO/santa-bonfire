...
export const HyperCardDetail = ({ blog, onBack, showBackButton = true, initialSectionId }: HyperCardDetailProps) => {
  ...
  // Full Content State
  const [fullCardContent, setFullCardContent] = useState<HyperBlogInfo | null>(blog.blog_content ? blog : null);
  ...
  const fetchFullContent = useCallback(async () => {
    if (fullCardContent?.blog_content) return;
    ...
    try {
      ...
      setFullCardContent(data);
    } catch (err: unknown) {
      console.error("Error loading full card content:", err);
      setError("Failed to load full card content. Please try again.");
      // Set preview content as fallback
      setFullCardContent(blog);
    } finally {
   ...
   */
  useEffect(() => {
    if (fullCardContent?.banner_url) {
      setBannerUrl(fullCardContent.banner_url);
  ...
  useEffect(() => {
    const handleIncrementView = async () => {
      ...
      try {
        const storedViews = sessionStorage.getItem("viewedCards");
        ...
        if (response.ok) {
          ...
          sessionStorage.setItem("viewedCards", JSON.stringify(Array.from(viewedSet)));
          setViewed(true);

          // Update view count locally
          const updatedCard = await response.json();
          setFullCardContent(prev => (prev ? { ...prev, view_count: updatedCard.view_count } : null));
        }
      } catch (err) {
   ...
   */
  useEffect(() => {
    if (initialSectionId && !isLoadingFullContent && fullCardContent?.blog_content?.sections) {
      // Small delay to ensure DOM is ready
      setTimeout(() => {
        handleScrollToSection(initialSectionId);
      }, 300);
    }
  }, [initialSectionId, isLoadingFullContent, fullCardContent, handleScrollToSection]);

  /**
   * Scroll tracking effect for TOC active state
   */
  useEffect(() => {
    if (!fullCardContent?.blog_content?.sections) {
      return;
    }
    ...
    const sections = fullCardContent.blog_content.sections.map(section => {
  ...
  const handleSubmitComment = useCallback(async () => {
    ...
    try {
      ...
      setFullCardContent(prev => (prev ? { ...prev, comment_count: (prev.comment_count || 0) + 1 } : null));
  ...
  useEffect(() => {
    const handleKeyboardNavigation = (event: KeyboardEvent) => {
      ...
      if (!fullCardContent?.blog_content?.sections) return;

      const sections = fullCardContent.blog_content.sections.sort((a, b) => a.order - b.order);
  ...
  }, [fullCardContent, currentSectionIndex, handleScrollToSection, onBack]);

  const currentCard = fullCardContent || blog;
          ...
          <>
            ...
            <div ref={tocRef} className="hidden lg:block lg:w-64 lg:flex-shrink-0">
              <div className="fixed top-[140px] w-64 h-[calc(100vh-160px)] overflow-y-auto border-r border-base-300 bg-base-100 z-10">
                <div className="p-4">
                  <h4 className="text-sm font-semibold uppercase tracking-wide opacity-70 mb-3">🎁 Card Sections</h4>
        ...
        <div ref={contentRef} className="flex-1 px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8 max-w-4xl mx-auto w-full">
          ...
          {!isLoadingFullContent && (
            <div className="max-w-3xl mx-auto">
              ...
              {currentCard.blog_content && currentCard.blog_content.sections ? (
                // Render full card sections
                <div className="space-y-8 lg:space-y-12">
                  ...
                  {currentCard.blog_content.metadata && (
                    <div className="bg-base-200 rounded-lg p-4 lg:p-6 space-y-3">
                      <h4 className="text-sm font-semibold uppercase tracking-wide opacity-70 mb-3">🎄 Card Info</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            <FileText className="w-4 h-4 opacity-60" />
                            <span>
                              <strong>Total Words:</strong> {currentCard.blog_content.metadata.total_words}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span>
                              ...
                              {currentCard.blog_content.metadata.generation_time_seconds}s
                            </span>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            <FileText className="w-4 h-4 opacity-60" />
                            <span>
                              <strong>Model:</strong> {currentCard.blog_content.metadata.model}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <FileText className="w-4 h-4 opacity-60" />
                            <span>
                              <strong>Sections:</strong> {currentCard.blog_content.metadata.sections_generated}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <>
                  <div className="prose prose-lg max-w-none mb-8">
                    <p className="text-base leading-relaxed whitespace-pre-wrap">{currentCard.preview}</p>
                  </div>
                  {currentCard.generation_status === "completed" && (
              ...
              <div className="mt-12 pt-8 border-t border-base-200">
                ...
                >
                  ...
                  >
                    ...
                    💬 Card Messages ({currentCard.comment_count || 0})
                  </div>
                  <div className="collapse-content">
                    <div className="space-y-4 mb-6 mt-2">
                      {loadingComments ? (
                        ...
                        <div className="text-center opacity-50 py-4">No messages yet. Be the first to send holiday wishes! 🎄</div>
                      )}
                    </div>
                    ...
                    {isConnected ? (
                      <div className="form-control">
                        <textarea
                          ...
                          placeholder="Send your holiday wishes..."
              ...
              {currentCard.generation_status === "completed" && (
                <div className="mt-8 flex justify-center gap-4">
                  ...
                  >
                    ...
                    <span>{currentCard.upvotes || 0}</span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span>{currentCard.downvotes || 0}</span>
                  </button>
                </div>
              )}
      ...
      {!isLoadingFullContent && (
        <div className="border-t border-base-300 px-4 sm:px-6 py-4 lg:py-6 bg-base-100 z-20">
          <div className="max-w-5xl mx-auto">
            <h4 className="text-sm font-semibold uppercase tracking-wide opacity-70 mb-3">🎄 Card Information</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
              <div className="bg-base-50 rounded-lg p-3">
                <div className="flex items-center gap-2 text-sm">
                  <User className="w-4 h-4 opacity-60" />
                  <span className="truncate">
                    <strong>Created by:</strong> {truncateAddress(currentCard.author_wallet, 8)}
                  </span>
                </div>
              </div>
              <div className="bg-base-50 rounded-lg p-3">
                <Link
                  href={`/data-rooms/${currentCard.dataroom_id}`}
                  className="flex items-center gap-2 text-sm hover:text-primary transition-colors group"
                  title="Visit this Bonfire to create your own card"
                >
                  <span className="text-base">🔥</span>
                  <span className="truncate">
                    <strong>From Bonfire:</strong>{" "}
                    {currentCard.dataroom_description || truncateAddress(currentCard.dataroom_id, 8)}
                  </span>
              ...
              <div className="bg-base-50 rounded-lg p-3">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>
                    <strong>Created:</strong> {formatTimestamp(currentCard.created_at)}
                  </span>
                </div>
              </div>
              {currentCard.word_count && (
                <div className="bg-base-50 rounded-lg p-3">
                  <div className="flex items-center gap-2 text-sm">
                    <FileText className="w-4 h-4 opacity-60" />
                    <span>
                      <strong>Words:</strong> {currentCard.word_count}
                    </span>
                  </div>
                </div>
              )}
              {currentCard.tx_hash && (
                <div className="bg-base-50 rounded-lg p-3">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <a
                      href={`https://etherscan.io/tx/${currentCard.tx_hash}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="link link-primary hover:underline flex items-center gap-1 truncate"
                    >
                      <strong>Transaction:</strong> {truncateAddress(currentCard.tx_hash, 6)}
                    </a>
                  </div>
                </div>
              )}
